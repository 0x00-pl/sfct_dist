...
let rec string_dec s x =
  ...
  | String (a, s0) ->
    (match x with
     | EmptyString -> false
     | String (a0, s1) -> if (=) a a0 then string_dec s0 s1 else false)
         ...
         (match firstExpect (String ('(', EmptyString)) (parseSumExp steps')
          ...
          | SomeE p ->
            ...
            (match expect (String (')', EmptyString)) rest with
             | SomeE p0 -> let Pair (_, rest') = p0 in SomeE (Pair (e, rest'))
             | NoneE err -> NoneE err)
          | NoneE err -> NoneE err)))
    steps
      ...
      (match expect (String ('f', (String ('a', (String ('l', (String ('s',
       ...
       | NoneE _ ->
         (match firstExpect (String ('n', (String ('o', (String ('t',
          ...
          | SomeE p -> let Pair (e, rest) = p in SomeE (Pair ((BNot e), rest))
          | NoneE _ ->
            (match firstExpect (String ('(', EmptyString))
