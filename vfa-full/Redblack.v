...
(** We will use the same framework as in Extraction.v:  keys are Ocaml
   integers. We don't repeat the [Extract] commands, because they are
   imported implicitly from Extraction.v *)
...
(** lookup is exactly as in our (unbalanced) search-tree algorithm in
  Extraction.v, except that the [T] constructor carries a [color] component,
...
(** This one is pretty easy, even without proof automation.
  Copy-paste your proof of insert_SearchTree from Extraction.v.
...
Proof.  (* Copy your proof from Extraction.v, and adapt it. *)
(* FILL IN HERE *) Admitted.
(** [] *)
...
(* ################################################################# *)
(** * Success! *)
(** The benchmark measurements above (and in Extraction.v) demonstrate that:
