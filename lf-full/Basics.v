...
(** 函数式编程风格让编程更接近简单的、日常的数学：若一个过程或方法没有副作用，
    ...
    函数式编程中「函数式」一词的另一个含义是它强调把函数（或方法）作为 _'第一等'_ 
    ...
    后半部分则介绍了可被用于证明 Coq 程序的简单属性的一些基本 _'策略（Tactic）'_ 。 *)

(* ################################################################# *)
(** * 可枚举类型 *)

(** Coq 一个不寻常的地方就是它内置了 _'极小'_ 的特性集合。比如，Coq 并未提供
...
(** 让我们从一个非常简单的例子开始，看看这种定义机制是如何工作的。
    下面的声明告诉 Coq 我们在定义一个新的数据值的集合，即一个 _'类型'_ 。 *)
...
(** 注意，这里显式声明了函数的参数和返回类型。像大多数函数式编程语言一样，
    如果没有显式指定类型，Coq 自己通常会通过 _'类型推断'_ 得出。
...
(** 一些细节问题我们暂且不谈（之后还会讲到），不过这段代码基本上可以读作
    ...
    第三，我们可以让 Coq 从 [Definition] 中 _'提取'_ 出一个用更常规的编程语言
    （如 OCaml、Scheme、Haskell）编写的程序，它们有着高性能的编译器。
    这种能力非常有用，我们可以通过它将 Gallina 编写的，
     _'证明正确'_ 的算法转译成高效的机器码。（当然，我们得相信 OCaml/Haskell/Scheme
...
(** _'关于记法的说明'_ ：在 [.v] 文件中，我们用方括号来界定注释中的 Coq 代码片段；
...
(** Coq 提供了 _'模块系统'_ 来帮助组织大规模的开发。在本课程中，
...
(** 让我们来更仔细地看一下这个定义。
    所有可归纳式定义的集合（[day]、[nat]、[bool] 等）实际上都是 _'表达式'_ 的集合。
...
(** 同样的规则也适用于 [day] 和 [bool] 的定义。（对于它们的构造器我们使用的标记
    ...
    之类的表达式则不属于 [nat]。
    关键的一点在于，我们目前只是定义了一种数字的 _'表示'_ 方式：一种写下它们的方式。
...
(** 第一行里的 _ 是一个 _'通配符'_ 。在模式匹配中使用 _ 就如同写一个变量但在
...
(** 我们可以通过引入加法、乘法和减法的 _'记法'_ 来让数字表达式更易读一些。 *)
...
(** [leb] 测试其第一个参数是否小于或等于第二个参数，并返回一个布尔值。 *)
...
(** 此外，[reflexivity] 在某些方面做了比 [simpl] _'更多'_ 的化简 ——
    ...
    和 [Remark]）都表示完全一样的东西。
    其次，我们增加了量词 [forall n:nat]，因此我们的定理讨论了 _'所有的'_ 自然数 [n]。
    ...
    关键字 [intros]、[simpl] 和 [reflexivity] 都是 _'策略'_ 的例子。
...
(** 在下一章里，我们会引入 _'归纳'_ 这种技术来证明这个目标。
    现在，让我们来看一些更简单的策略。 *)
...
(* （注意，此证明其实不用 [rewrite] 就能证明，不过请使用 [rewrite] 来做练习。 *)
(** [] *)
...
(** 每一对 [reflexivity] 调用对应于紧邻其上的 [destruct] 行执行后所生成的子目标。 *)
...
(** 对于 Coq 中的每个记法符号，我们可以指定它的 _'优先级'_ 和 _'结合性'_ 。    优先级 [n] 用 [at level n] 来表示，这将有助于 Coq 分析复合表达式。    结合性的设置有助于消除有相同符号出现多次的表达式的歧义。比如，    上面这组对 [+] 和 [*] 的参数定义的表达式 [1+2*3*4] 是 [(1+((2*3)*4))] 的    简写。Coq 使用 0 到 100 的优先级等级，同时支持 _'左结合'_ 、 _'右结合'_ 和 _'不结合'_     三种结合性。之后我们会看到更多与此相关的例子，比如在 "列表"([Lists]) 一章。    每个记法符号还与 _'记法范围(notation scope)'_ 相关。Coq 会尝试根据上下文来猜测    你所指的范围，因此当你写出 [S(0*0)] 时，它猜测是 [nat_scope]；而当你    写出笛卡尔积（元组）类型 [bool*bool] 时，它猜测是 [type_scope]。    有时你可能不得不用百分号记法写出 [(x*y)%nat] 来帮助 Coq 确定范围，    另外，有时 Coq 对你的反馈中也包含 [%nat] 用来指示记法所在的范围。    记法范围同样适用与数字表示（[3]、[4]、[5] 等等），因此你有时候会看到    [0%nat]，表示 [0]（我们在本章中使用的自然数零 [0]），而 [0%Z] 表示整数零    （来自于标准库中的另一个部分）。    专业提示：Coq 的符号机制不是特别强大。别期望太多！ *)
...
(** 当 Coq 查看此定义时，它会意识到 [plus'] 是「在其第一个参数上递减」。
    这意味着我们在参数 [n] 上执行了 _'结构化递归'_ 。换言之，我们仅对严格减小了的
...
(** 为了能对此有更具体的认识，找出一种方式写出有效的 [Fixpoint] 定义
    (比如有关数字的简单函数)，在各种的输入下应当 _'确实'_ 能够终止，但是 Coq
    却受限于此而拒绝接受。 *)
...
(** $Date: 2017-03-06 06:40:22 -0800 (Mon, 06 Mar 2017) $ *)
